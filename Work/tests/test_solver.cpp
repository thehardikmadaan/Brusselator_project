/*
Test suite for the Brusselator problem solver.
Includes tests for matrix correctness, mass conservation,
solver agreement, and comparison with a reference solution.
*/
...
// Test 1: Verify that the row sums of the Laplacian      \\
// matrix A are exactly 0 (Neumann BCs)                   \\
...
// Test 2: Verify mass conservation when reactions are    \\
// disabled                                               \\
...
void testMassConservation() {
  ...
  // Add some random perturbations to make it           \\
    // interesting                                        \\
  ...
  // Mass should be conserved exactly (up to tiny       \\
    // rounding errors)                                   \\

  if (std::abs(final_mass1 - initial_mass1) > 1e-10 ||
      std::abs(final_mass2 - initial_mass2) > 1e-10) {
    std::cerr << "\nFAIL: Mass not conserved!\n"
              << "  C1 Initial: " << initial_mass1
              << " | Final: " << final_mass1 << "\n"
              << "  C2 Initial: " << initial_mass2
              << " | Final: " << final_mass2 << "\n";
    std::exit(1);
  }
...
// Test 3: Verify that RK4 and Implicit Euler agree for a \\
// small time step                                        \\
...
void testSolverAgreement() {
  ...
  // For dt = 0.001, RK4 (O(dt^4)) and Implicit Euler   \\
    // (O(dt)) should be extremely close                  \\

  if (diff > 1e-5) {
    std::cerr << "\nFAIL: Solvers disagree! Max difference: " << diff
              << " (expected < 1e-5)\n";
    std::exit(1);
  }
...
void testReferenceSolution() {
  ...
  if (!refGrid || refGrid->GetNumberOfPoints() == 0) {
    std::cerr << "\nFAIL: Could not load reference file! (Checked path: "
                 "../final_output_30s (1).vts)\n";
    std::exit(1);
  }

  vtkFloatArray *c1_ref =
      vtkFloatArray::SafeDownCast(refGrid->GetPointData()->GetArray("C1"));
  vtkFloatArray *c2_ref =
      vtkFloatArray::SafeDownCast(refGrid->GetPointData()->GetArray("C2"));
  ...
  if (max_diff > 1e-4) {
    std::cerr << "\nFAIL: Solution differs from reference! Max difference: "
              << max_diff << "\n";
    std::exit(1);
  }
