/*
 implement the spatial discretization
 and the solver for the Brusselator equations
*/
...
buildMatrixAtilde(const GridInfo &grid, const Eigen::SparseMatrix<double> &A) {
  ...
  for (int k = 0; k < A.outerSize(); ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(A, k); it; ++it) {
      ...
      triplets.push_back(
          Eigen::Triplet<double>(it.row(), it.col(), grid.D1 * it.value()));
...
buildJacobian(const GridInfo &grid, const Eigen::SparseMatrix<double> &Atilde,
  ...
  if (grid.reactions) {
    for (int p = 0; p < N; ++p) {
      ...
      triplets.push_back(
          Eigen::Triplet<double>(p + N, p + N, -grid.dt * d_w2_dc2));
    }
  }
