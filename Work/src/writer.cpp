/*
Implement an XML VTK (.vts) structured points
exporter using the vtk9 library, enabling the concentration fields C1 and C2 to
be saved compactly and efficiently for visualization in ParaView or PyVista.
*/
...
void writeVTK(const std::string &filename, const GridInfo &grid,
  ...
  vtkSmartPointer<vtkFloatArray> c1Array =
      vtkSmartPointer<vtkFloatArray>::New();
  c1Array->SetName("C1");
  c1Array->SetNumberOfComponents(1);
  c1Array->SetNumberOfTuples(N);

  vtkSmartPointer<vtkFloatArray> c2Array =
      vtkSmartPointer<vtkFloatArray>::New();
  c2Array->SetName("C2");
