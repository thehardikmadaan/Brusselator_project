/*
Main solver file.
Calls other functions to set up the problem and solve it.
Handles command-line argument parsing, initializes the
state vectors, coordinates the time-stepping loop, and manages the file-writing
interval.
*/
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 7) {
    ...
    std::cerr << "Usage (7 positional arguments):\n"
              ...
              << "  " << argv[0]
              << " <nx> <ny> <duration> <write_interval> <Ca> <Cb> [method] "
                 "[reactions]\n";
    return 1;
  }
  ...
  std::cout << "Domain: " << (grid.nx - 1) * grid.dx << "m x "
            << (grid.ny - 1) * grid.dy << "m\n";
  std::cout << "Method: "
            << (grid.method == "implicit"
                    ? "Implicit Euler (Newton)"
                    : (grid.method == "explicit" ? "Explicit Euler"
                                                 : "Explicit RK4"))
            << "\n";
...
// Create output directory                            \\
  ...
  while (t < grid.duration) {
    ...
    if (grid.method == "implicit") {
      ...
      if (step % 10 == 0 || t >= grid.duration) {
        std::cout << "Time: " << std::fixed << std::setprecision(3) << t << " s"
                  << " | Step: " << step << " | Newton Iterations: " << step_its
                  << "\n";
      }
    ...
    if (t - last_write_time >= grid.write_interval - 1e-9 ||
        t >= grid.duration) {
      std::stringstream ss;
      ss << "output/output_" << std::setfill('0') << std::setw(4)
         << file_counter << ".vts";
