  ...
  for (int i = 0; i < params.ny; ++i) {
    for (int j = 0; j < params.nx; ++j) {
      ...
      triplets.push_back(Eigen::Triplet<double>(
          k, k, -static_cast<double>(active_neighbors) * factor));
    }
  }
...
buildMatrixAtilde(const SimParams &params,
  ...
  for (int k = 0; k < A.outerSize(); ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(A, k); it; ++it) {
      ...
      triplets.push_back(
          Eigen::Triplet<double>(it.row(), it.col(), params.D1 * it.value()));
      // Bottom-right block: D2 * A (inhibitor)
      triplets.push_back(Eigen::Triplet<double>(it.row() + N, it.col() + N,
                                                params.D2 * it.value()));
    }
  }
...
Eigen::VectorXd computeRHS(const SimParams &params,
...
void stepRK4(const SimParams &params, const Eigen::SparseMatrix<double> &Atilde,
             Eigen::VectorXd &C) {
  double dt = params.dt;
...
// Evaluate the residual g(C_new) = C_new - C_old - dt * (Atilde * C_new +
// omega(C_new))
Eigen::VectorXd computeResidual(const SimParams &params,
                                const Eigen::SparseMatrix<double> &Atilde,
...
Eigen::SparseMatrix<double>
buildJacobian(const SimParams &params,
              const Eigen::SparseMatrix<double> &Atilde,
              const Eigen::VectorXd &C) {
  int N = params.nx * params.ny;
  ...
  for (int p = 0; p < N; ++p) {
    ...
    triplets.push_back(
        Eigen::Triplet<double>(p + N, p + N, -params.dt * d_w2_dc2));
  }
...
void stepImplicitEuler(const SimParams &params,
                       const Eigen::SparseMatrix<double> &Atilde,
  ...
  if (!converged) {
    std::cerr << "Newton's method failed to converge! Residual norm: "
              << computeResidual(params, Atilde, C_new, C_old)
...
int main(int argc, char *argv[]) {
  // We allow an optional 8th argument to specify the solver method ("rk4" or
  // "implicit")
  ...
  if (argc == 9) {
    ...
    std::cerr << "Usage: " << argv[0]
              << " <nx> <ny> <duration> <dt> <write_interval> <Ca> <Cb> "
                 "[<method: rk4|implicit>]\n";
    return 1;
  }
  ...
  std::cout << "Method: "
            << (method == "implicit" ? "Implicit Euler (Inexact Newton)"
                                     : "Explicit RK4")
            << "\n";
  std::cout << "Grid: " << params.nx << "x" << params.ny << "\n";
  std::cout << "Domain: " << (params.nx - 1) * params.dx << "m x "
            << (params.ny - 1) * params.dy << "m\n";

  // Initialize data structures
  int num_points = params.nx * params.ny;
  Eigen::VectorXd C1 = Eigen::VectorXd::Constant(num_points, params.Ca);
  Eigen::VectorXd C2 =
      Eigen::VectorXd::Constant(num_points, params.Cb / params.Ca);
